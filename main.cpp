...
int main(int argc, const char** argv)
{
	...
	location.y += line_step*3/2;
	putText( default_image, "2. Images (Sampling+Quantisation, Colour Models, Noise+Smoothing)", location, FONT_HERSHEY_SIMPLEX, 0.4, colour );
	location.y += line_step;
	putText( default_image, "3. Histograms (Histograms, Equalisation, Selection, Back Proj, k-Means)", location, FONT_HERSHEY_SIMPLEX, 0.4, colour );
	location.y += line_step;
	putText( default_image, "4. Binary Vision (Thresholding, Morphology, Connected Components)", location, FONT_HERSHEY_SIMPLEX, 0.4, colour );
	location.y += line_step;
